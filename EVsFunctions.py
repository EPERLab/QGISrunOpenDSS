...
Para lograr obtener este diccionario se debe leer un dss con la información del VE.
Para ello se parte del hecho de que las líneas de dicho archivo tendrán el siguiente formato:
...
*evs_power (dataframe): representa los kW aleatorios de los VEs.
Contiene una cantidad de total_cars elementos
*evs_initial_soc (dataframe): dataframe que representa el estado
 de carga inicial de los VEs. Contiene una cantidad de total_cars elementos
*evs_kwh (dataframe): representa los kW aleatorios de los VEs. Contiene
una cantidad de total_cars elementos
"""
def CreacionPerfilesEV(power_val, kwh_val, total_cars = 100):
    ...
    evs_loadshapes = pd.DataFrame(0, index=[x for x in range(0,96)],
								  columns=['curve_'+str(x) for x in range(1,total_cars+1)])
    evs_initial_soc = pd.DataFrame(np.nan, index=[x for x in range(0,2)],
								   columns=['soc_ev_'+str(x) for x in range(1,total_cars+1)])
    evs_power = pd.DataFrame(0, index=[x for x in range(0,1)],
							 columns=['pow_ev_'+str(x) for x in range(1,total_cars+1)])
    evs_kwh = pd.DataFrame(0, index=[x for x in range(0,1)],
						   columns=['kwh_ev_'+str(x) for x in range(1,total_cars+1)])
